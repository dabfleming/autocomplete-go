...
class GocodeProvider {
  ...
  getSuggestions (options) {
    let p = new Promise((resolve) => {
      ...
      let offset = Buffer.byteLength(text.substring(0, index - prefixLength), 'utf8')
      ...
      config.locator.findTool('gocode', locatorOptions).then((cmd) => {
        ...
        config.executor.exec(cmd, args, {cwd: cwd, env: env, input: text}).then((r) => {
          ...
          if (r.stdout && r.stdout.trim() !== '') {
            messages = this.mapMessages(r.stdout, options.editor, options.bufferPosition, options.prefix)
          }
  ...
  mapMessages (data, editor, position, prefix) {
    if (!data) {
      return []
    }
    let res
    ...
    // Filter by fuzzy search before processing
    const query = (prefix === '.' ? '' : prefix)
    candidates = filter(candidates, query, {key:"name"})
